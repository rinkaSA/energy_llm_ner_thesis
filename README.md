...
│   ├── src/ 
│       ├──models/               # load the models snapshots in this dir, containt example loading function
...
│       │   │    │    ├── xtreme_de_batch_experiment.py  # experiment #1 -> finding the trade off between batch size and energy used
│       │   │    ├── eval_llm_batches.py
│       │   │    ├── simple_gollie_eval.py
│       │   ├── gpu_metrcis_server/       # the time series with gpu metric are stored here
│       │   ├── inference_eval_artifacts/ # the results of evaluation of exp #2 stored here (& mlflow)
...
│       │   ├── two_node_serve_depricated/   # monitoring and serving on 2 separated nodes, old
│       │   ├── visualizers/  # function for exp #1 - metrics vs batch size box plots
│       │   ├── collect_gpu_nvml.py     # Script to collect GPU metrics via NVML endpoint continious scraping
│       │   ├── README_metrics.md   # detailed overview on vllm, gpu metrics
│       │   ├── README_one_node.md  # a bit older version of detaield explnantion of one node serve of the model.
```
...
- /src/serve_vllm/SERVE_model_bash/<model_name>/one_node_serve_<model_name>.sh: Sets up the backbone — a vLLM server hosting the Mistral/Geema 3/ GoLLIE model, wrapped with a complete monitoring stack (Prometheus, Grafana, Jaeger, DCGM) to track GPU utilization and energy metrics in real time. I use H100-80GB one node, so the models are up to 12B to fit it nicely.
...
Evaluation of one language for NER task with simple prompt for a list of batch sizes ( from 1 to 128) while capturing energy metrics to showcase that having big enough batch size we spend less energy per token (utilizing GPU parallelization).

### Experiment 2 

Evaluation of all selected languages for a defined batch size with 8 types of prompt. More details below.
...
1. **F1** : accuracy score has to be relatively high.

## Running Experiments

### Step 1. Prerequisites
1. Set up environment variables in /serve_vllm/.env file (template_provided)
...
3. Pull docker container desined for vllm serve.
...
### Step 3. Check if the .env file have a node name updated based on the assigned node for the previous task.

### Step 4. Send the simple request with CONNL03 dataset.

This is needed to prefill vllm metrics with data, as in my main experiment #1 and #2 logic we scrape for metrics BEFORE request is sent and after. For before to work properly (no NaNs), I have a _base_ script without metric capturing function, just a simple request.
``` sbatch SERVE_model_bash/gemma/inference_base_gemma.sh```

### Step 5. Experiemnt #2 execution
...
Each srun executes inference pass of selected language (using its whole test split of the dataset), parses the output, BIO tags, compares gold tags and predicted, calculates F1 score (overall & per entity category), while capturing energy. and vllm metrics for each batch and summarazies it, all produced info is. mlflow logged.
## To get the idea about the prompt styles, have a look at ```prompts_in_all_languages/Template_system_prompt.json```. The repeating prompts are combined with varying amount of examples (few-shot prompting). 
