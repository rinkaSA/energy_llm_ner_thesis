...
Firstly run ```sbatch inference.py``` while inside the ```python eval_llm_batches.py```
then the important metrics from vllm will be defined (kinda spun up) and we can use the script where we have the calculations on energy and vllm metrics performed.

Do ```sbatch inference.py``` while inside the ```nomlflow_batched_eval_energy.py``` (no mlflow logging since some time ago there was a problem with that, if it is working already, use ```batched_eval_energy.py```)
