...
def evaluate_and_log(
    all_true_labels, all_predicted_labels, comparisons, artifact_prefix
):
    """
    ...
    results = seqeval_metric.compute(
        predictions=all_predicted_labels, references=all_true_labels
    )

    # Log overall metrics without code duplication
    overall_metrics = [
        "overall_precision",
        "overall_recall",
        "overall_f1",
    ...
    for metric in overall_metrics:
        if metric in results:
            ...
            metrics_dict[metric] = results[metric]
    ...
    for entity_type in unique_labels:
        true_binary = [
            entity_type if label == entity_type else "Other" for label in true_flat
        ]
        pred_binary = [
            entity_type if label == entity_type else "Other" for label in pred_flat
        ]
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm_normalized, display_labels=[entity_type, "Other"]
        )
        disp.plot(ax=ax, cmap=plt.cm.Blues, values_format=".2%")
        plt.title(f"Confusion Matrix for Entity Type: {entity_type}")
        plt.tight_layout()

        per_type_filename = cm_filename.replace(
            "confusion_matrix", f"{entity_type}_confusion_matrix"
        )
...
def plot_all_entity_confusion_matrices(
    true_labels_list, pred_labels_list, entity_types
):
    """
...
def prepare_dataset(dataset_name):
    ...
    for sample in dataset:
        ...
        samples.append(
            {"tokens": tokens, "joined_text": joined_text, "true_labels": true_labels}
        )
    return samples
...
def get_model_pipeline(model_type, model_path):
    ...
    if model_type == "roberta":
        return pipeline(
            "ner", model=model_path, tokenizer=model_path, aggregation_strategy="simple"
        )
    elif model_type == "universalner":
        return pipeline(
            "text-generation", model=model_path, torch_dtype=torch.float16, device=0
        )
    raise ValueError(f"Unsupported model type: {model_type}")
...
def process_batch(batch, model_type, model_pipeline, args):
    ...
    if model_type == "roberta":
        ...
        outputs = model_pipeline(
            inputs, max_new_tokens=args.max_new_tokens, return_full_text=False
        )
...
def run_ner(args):
    ...
    with mlflow.start_run(run_name=run_name, log_system_metrics=True):
        ...
        results = evaluate_and_log(
            ...
            all_predicted_labels,
            comparisons,
            artifact_prefix=args.model_type,
        )
...
def main():
    ...
    parser.add_argument(
        "--run_name", type=str, default="default_run", help="MLflow run name"
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="Universal-NER/UniNER-7B-type",
        help="Model path for NER",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
        default=256,
        help="Max tokens to generate (for UniversalNER)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_type",
        type=str,
        choices=["roberta", "universalner"],
        default="universalner",
