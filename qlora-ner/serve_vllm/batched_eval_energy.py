...
ENERGY_METRIC_NAME    = "DCGM_FI_DEV_TOTAL_ENERGY_CONSUMPTION"  # adjust if yours differs


def read_energy_joules() -> float:
    """Scrape the DCGM exporters gauge from localhost:9400/metrics."""
    r = requests.get("http://i8001.alpha.hpc.tu-dresden.de:9400/metrics", timeout=1.0).text
...
VLLM_METRICS_URL = "http://i8001.alpha.hpc.tu-dresden.de:8000/metrics"
...
async def process_and_measure(session, prompts, max_tokens):
    ...
    telemetry = {
        ...
        "joules_per_token":     joules / total_t,

        # server‐measured means
...
async def main():
    ...
    for B in [ 32]: # [1, 4, 8, 16, 32, 64]:
        ...
        with mlflow.start_run(run_name=run_name, log_system_metrics=True) as run:
            ...
            with open(f"server_response_results/telemetry_B{B}.json", "w") as f:
                json.dump(batch_telemetry, f, indent=2)
